...
const uploadRemoteDoodServer = async (linkSource, title) => {
  ...
  let isExisted = listFiles.find(
    (f) => encodeURIComponent(f.title) == encodeURIComponent(title)
  );
  ...
  let apiUpload = config.DOOD_UPLOAD_API.replace("[URL]", linkSource).replace(
    "[TITLE]",
